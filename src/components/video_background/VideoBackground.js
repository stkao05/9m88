...
import { filterOptions as FILTER_OPTIONS } from "./filter.js"
import { filterProperties as FILTER_PROPERTIES } from "./filter.js"
...
const testAutoPlay = () => Promise.resolve()
...
class VideoBackground {
  ...
  bindUI() {
    ...
    const resizeHandler = () => {
      if (resizeEvent === "resize" && this.player.iframe) {
        ...
        this.useCustomFallbackImage &&
        this.windowContext.ImageLoader
      ) {
        const customFallbackImage = this.container.querySelector(
          "img[data-src]"
        )
  ...
  initializeProperties(props = {}) {
    ...
    this.playbackSpeed =
      parseFloat(props.playbackSpeed) === 0.0
        ? 1
        : parseFloat(props.playbackSpeed)
    this.timeCode = {
  ...
  initializeYouTubeAPI() {
    ...
    if (
      this.windowContext.document.documentElement.querySelector(
    ...
    const firstScriptTag = this.windowContext.document.getElementsByTagName(
      "script"
    )[0]
  ...
  initializeYouTubePlayer() {
    ...
    const onYouTubePlayerStateChange = event => {
      ...
      const duration =
        (player.getDuration() - this.timeCode.start) / this.playbackSpeed
      ...
      if (
        event.data === this.windowContext.YT.PlayerState.BUFFERING &&
        player.getVideoLoadedFraction() !== 1 &&
        (player.getCurrentTime() === 0 ||
          player.getCurrentTime() > duration - -0.1)
      ) {
  ...
  initializeVimeoPlayer() {
    ...
    playerIframe.src =
      "//player.vimeo.com/video/" + this.videoId + "?api=1" + playerConfig
    ...
    const postMessageManager = (action, value) => {
      ...
      this.windowContext.eval(
        "(function(ctx){ ctx.player.iframe.contentWindow.postMessage(" +
          message +
          ", " +
  ...
  getFilterStyle(filter, strength) {
    return `${filter}(${FILTER_PROPERTIES[filter].modifier(strength) +
      FILTER_PROPERTIES[filter].unit})`
  }
  ...
  _getTimeParameter(parsedUrl) {
    if (
      (this.videoSource === "youtube" &&
        (!parsedUrl.query || !parsedUrl.query.t)) ||
